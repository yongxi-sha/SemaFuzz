...
static bool closeFuncCoverage = false;
...
public:
    ...
    inline void InjectOne (IRBuilder<> &IRB, unsigned FuncKey, unsigned FuncID) 
    {
        ...
        Value *KeyVal = ConstantInt::get(Int32Ty, FuncKey, false);       
        Value *IDVal  = ConstantInt::get(Int32Ty, FuncID, false);

        CallInst* CI = IRB.CreateCall(ScanCovFunctionID, {NullPtr, KeyVal, IDVal});
        if (CI == NULL) {
          std::cout<<"create function call fail: FuncID = "<<FuncID<<std::endl;
          return;
        }
    ...
    inline void RunInject (unsigned FuncID) 
    {
        ...
        if (CurFunc->getName().str() == "main"){
          InjectOne (IRB, TARGET_EXIT_KEY, FuncID);
        }
        InjectOne (IRB, TARGET_BEGIN_KEY, FuncID);
        std::cout<<"->["<<FuncID<<"]: "<<"Instrument "<<CurFunc->getName().str()<<" begin."<<std::endl;

        for (BasicBlock &BB : *CurFunc) {
            for (Instruction &Inst : BB) {
                // Identify call to exit()
                if (CallInst *Call = dyn_cast<CallInst>(&Inst)) {
                    if (Function *Callee = Call->getCalledFunction()) {
                        if (Callee->getName().str() == "exit") {
                            CallInst* CI = CallInst::Create(TargetExitFunction, "", &Inst);
                            CI->setCannotMerge();
                            std::cout<<"@@Create __analysis_trace_exit @exit"<<std::endl;
                        }
                    }
                }
                ...
                if (isa<ReturnInst>(Inst)) {
                    //IRBuilder<> IRB(&Inst);
                    //InjectOne (IRB, TARGET_END_KEY, FuncID);
                    //std::cout<<"->["<<FuncID<<"]: "<<"Instrument "<<CurFunc->getName().str()<<" end."<<std::endl;

                    if (CurFunc->getName() == "main") {
                        CallInst* CI = CallInst::Create(TargetExitFunction, "", &Inst);
                        CI->setCannotMerge();
                        std::cout<<"@@Create __analysis_trace_exit @return of main"<<std::endl;
                    }
               }
            }
        }
...
public:
  ...
  TrcFunction (unsigned ANum, unsigned BNum, unsigned IdNo)
  {  
    ArgNum = ANum;
    BlkNum = BNum;
    ID     = IdNo;
...
class ModuleSanitizerCoverageAFL
 ...
 public:
  ModuleSanitizerCoverageAFL(
      ...
      : Options(OverrideFromCL(Options)) {
        closeFuncCoverage = (getenv ("FUNCOVERAGE_OFF") != NULL);
  }
 ...
 private:
  ...
  std::map<std::string, TrcFunction*> Function2ID;
  unsigned InitID = 1;
  std::string AIFunctionFile = "functions_list";
  ...
  inline void loadFunctions ()
  {
      ...
      char* CusFL = getenv ("FUNCTIONS_LIST");
      if (CusFL != NULL){
        AIFunctionFile = std::string (CusFL);
      }

      std::ifstream inputFile(AIFunctionFile);
      if (!inputFile) {
        return;
      }

      std::string FuncName;
      while (std::getline(inputFile, FuncName)) {
        std::vector<std::string> tokens = splitString (FuncName, ':');
        if (tokens.size() < 4) continue;

        FuncName = tokens[0];
        if (Function2ID.find (FuncName) != Function2ID.end())
            continue;
        Function2ID[FuncName] = new TrcFunction(std::stoi (tokens[1]), std::stoi (tokens[2]), std::stoi (tokens[3]));
        //std::cout<<"@@ --> load function: " + FuncName + " with ID: " + tokens[1] + "\n";
      }

    inputFile.close();
    InitID = (unsigned)Function2ID.size()+1;
  }
  ...
  inline void dumpFunctions() 
  {
      ...
      std::ofstream Fstm(AIFunctionFile);
      if (!Fstm) return;

      for (auto Itr = Function2ID.begin (); Itr != Function2ID.end(); Itr++) {
          TrcFunction *TF = Itr->second;
          Fstm << Itr->first + ":" + std::to_string(TF->ArgNum) + ":" + std::to_string(TF->BlkNum) + ":" + std::to_string(TF->ID) + "\n";
  ...
  inline unsigned getBBNum (Function &F)
  {
    return F.size();
  }

  inline bool isFiltered (std::string FuncName)
  {
    char* FilterFiles = getenv ("FUNC_EXCEP");
    if (FilterFiles == NULL){
      return false;
    }

    return (strstr (FilterFiles, FuncName.c_str()) != NULL);
  }

  inline unsigned getFuncID (Module &M, Function &F)
  {
      std::string FuncName = getFilename(M.getName().str()) + "_" + F.getName().str();
      if (isFiltered (FuncName)) {
        return 0;
      }

      auto Itr = Function2ID.find (FuncName);
      if (Itr == Function2ID.end()) {
        unsigned FuncId = InitID++;
        Function2ID[FuncName] = new TrcFunction (F.arg_size(), getBBNum(F), FuncId);
        return FuncId;
      }
      else {
        TrcFunction *TF = Itr->second;
