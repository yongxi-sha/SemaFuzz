...
void log_perf_periodic(int signum)
{
    ...
    if (first_crt != 0)
    {
        fprintf (Pef, "real_map_size: %u\n", afl_ref->fsrv.real_map_size);
        fprintf (Pef, "loging--time, explored-paths, covered-blocks, crashes\n");
    }
    ...
    fprintf (Pef, "%-12s, %-14u, %-14u, %-u\n", GetCurTime (), afl_ref->queued_items, block_num, (u32)afl_ref->total_crashes);
    fclose (Pef);
