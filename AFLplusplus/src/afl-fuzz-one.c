...
static inline bool is_read_done (u8* path)
{
  u8 path_buf[PATH_MAX];
  snprintf(path_buf, PATH_MAX, "%s/read_done", path);
  return (access(path_buf, F_OK) == 0);
}

static inline void tag_read_done (u8* path)
{
  u8 path_buf[PATH_MAX];
  snprintf(path_buf, PATH_MAX, "%s/read_done", path);
...
static unsigned lastReadTime = 0;
void read_semantics_seeds (afl_state_t *afl)
{
  if (time (NULL) - lastReadTime <= 60)
  {
    return;
  }
  lastReadTime = time (NULL);

  FILE *fLog = fopen ("semseed.log", "a+");
  unsigned dirNo = 0;
  u8 path_buf[PATH_MAX];

  while (true)
  {   
    snprintf(path_buf, PATH_MAX, "in/semantic%u", dirNo);
    if (access(path_buf, F_OK) != 0)
    {
      fprintf (fLog, "@@read_semantics_seeds: path -> %s, not exist\r\n", path_buf);
      break;
    }

    if (is_read_done (path_buf))
    {
      dirNo++;
      continue;
    }

    tag_read_done (path_buf);
    unsigned curSeedNum = afl->queued_items;
    read_testcases (afl, path_buf);
    fprintf (fLog, "@@read_semantics_seeds [%s] done: queued_items %u--> %u....\r\n", 
             path_buf, curSeedNum, afl->queued_items);
    break;
  }
