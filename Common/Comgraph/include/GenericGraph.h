...
public:
    ...
    inline EdgeTy* GetIncomingEdge(EdgeTy* InEdge) 
    {
        iterator it = m_InEdgeSet.find(InEdge);
        if(it == m_InEdgeSet.end())
        {
    ...
    inline EdgeTy* GetOutgoingEdge(EdgeTy* OutEdge) 
    {
        iterator it = m_OutEdgeSet.find(OutEdge);
        if(it == m_OutEdgeSet.end())
        {
...
public:
    ...
    inline VOID UpdateNodeId (NodeTy* node, DWORD id)
    {
        auto it = m_IDToNodeMap.find(node->GetId());
        assert(it != m_IDToNodeMap.end());        
