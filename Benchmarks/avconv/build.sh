...
function wllvm_compile ()
{
	...
	tar -xvf $target.tar.gz && cp Makefile $target/

	cd $target
	./configure --disable-yasm --disable-shared --cc=$CC --extra-cflags="-O0 -Xclang -disable-O0-optnone -g -save-temps=obj -fno-discard-value-names -w"
	make clean && make
	cd -

	svf_run avconv $target
}
